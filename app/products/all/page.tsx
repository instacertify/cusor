  ...
  return (
    <div className="mx-auto max-w-7xl px-4 sm:px-6 py-10">
      ...
      <form
        action="/products/all"
        method="GET"
        className="mt-8 bg-white rounded-2xl border border-cream-300 shadow-card p-4 grid gap-3 lg:grid-cols-[1.4fr_1fr_1fr_0.7fr_0.8fr_auto]"
      >
        <input
          type="text"
          name="q"
          defaultValue={sp.q ?? ""}
          placeholder="Product, IS standard or HSN code…"
        ...
        <select name="scheme" defaultValue={sp.scheme ?? ""} className="rounded-xl border border-cream-300 px-3 py-2.5 text-sm bg-white outline-none">
          <option value="">Scheme</option>
          <option value="ISI">ISI</option>
          <option value="CRS">CRS</option>
        </select>
        <select name="sort" defaultValue={sp.sort ?? "labs"} className="rounded-xl border border-cream-300 px-3 py-2.5 text-sm bg-white outline-none">
          {SORTS.map((s) => (
      ...
      {/* Table */}
      <div className="mt-4 bg-white rounded-2xl border border-cream-300 shadow-card overflow-x-auto">
        <table className="w-full text-sm min-w-[980px]">
          <thead>
            <tr className="text-left text-xs uppercase tracking-wide text-ink-500 border-b border-cream-200 bg-cream-50">
              <th className="px-4 py-3.5 font-bold">Product (IS Title)</th>
              <th className="px-4 py-3.5 font-bold">IS Standard</th>
              <th className="px-4 py-3.5 font-bold">HSN</th>
              <th className="px-4 py-3.5 font-bold">QCO Status</th>
              <th className="px-4 py-3.5 font-bold text-right">Marking Fee (Lg)</th>
              <th className="px-4 py-3.5 font-bold text-right">Test Cost</th>
              <th className="px-4 py-3.5 font-bold text-right">Labs</th>
            </tr>
          </thead>
          <tbody>
            {products.map((p) => (
              <tr key={p.id} className="border-b border-cream-100 last:border-0 hover:bg-cream-50">
                ...
                <td className="px-4 py-3">
                  {p.qco_status ? (
                    <span
                      className={`inline-block text-[11px] font-bold rounded-full px-2.5 py-0.5 whitespace-nowrap ${
            ...
            {products.length === 0 && (
              <tr>
                <td colSpan={7} className="px-4 py-10 text-center text-ink-500 text-sm">
                  ...
                  <Link href="/contact" className="font-bold text-butter-700">ask an expert</Link>.
                </td>
              </tr>
            )}
          </tbody>
