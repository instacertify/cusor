...
async function saveGdprSettingsAction(formData: FormData) {
  "use server";
  await requireAdmin();
  saveGdprAdminSettings({
  ...
  return (
    <div>
      ...
      <p className="text-ink-600 text-sm mb-6 max-w-2xl">
        Privacy management — cookie consent banner, analytics gating, data-subject /
        Data Principal requests, consent log and inquiry retention. Public guide:{" "}
      ...
      <SavedBanner saved={sp.saved} message="Saved — GDPR settings / request updated." />
      {sp.redacted != null ? (
      ...
      <div className="grid sm:grid-cols-3 gap-4 mb-8">
        ...
        <div className="rounded-2xl border border-cream-300 bg-white p-5">
          ...
          <p className="mt-1 font-display text-xl font-semibold text-ink-950">
            {settings.bannerEnabled ? "Enabled" : "Disabled"}
          </p>
        </div>
        <div className="rounded-2xl border border-cream-300 bg-white p-5">
          <p className="text-[11px] font-bold uppercase tracking-wide text-ink-500">
          ...
          <p className="mt-1 font-display text-xl font-semibold text-ink-950">
            {settings.requireAnalyticsConsent ? "Required" : "Not required"}
          </p>
        </div>
      </div>
      ...
      <section className="bg-white rounded-2xl border border-cream-300 shadow-card p-6 mb-8">
        ...
        <form action={saveGdprSettingsAction} className="space-y-4 max-w-2xl">
        ...
        <form action={runRetentionAction} className="mt-6 pt-6 border-t border-cream-200">
          <p className="text-sm text-ink-600 mb-3">
            ...
            (name/email/phone/message redacted).
          </p>
          <SubmitButton
      ...
      <section className="mb-8">
        ...
        {requests.length === 0 ? (
          <p className="text-sm text-ink-500 bg-white rounded-2xl border border-cream-300 p-6">
            ...
            <Link href="/privacy/data-request" className="font-semibold text-butter-700 hover:underline">
