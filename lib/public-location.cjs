...
const ACTION_REDIRECT_TYPES = new Set(["push", "replace"]);
...
/** Next Server Actions send `x-action-redirect: <url>;push|replace`. */
function toPublicActionRedirect(value) {
  const raw = Array.isArray(value) ? value[0] : value;
  if (typeof raw !== "string" || raw.length === 0) return value;
  const semi = raw.lastIndexOf(";");
  if (semi > 0 && ACTION_REDIRECT_TYPES.has(raw.slice(semi + 1))) {
    return `${toPublicLocation(raw.slice(0, semi))};${raw.slice(semi + 1)}`;
  }
  return toPublicLocation(raw);
}

function rewriteRedirectHeader(name, value) {
  const key = String(name).toLowerCase();
  if (key === "location") return toPublicLocation(value);
  if (key === "x-action-redirect") return toPublicActionRedirect(value);
  return value;
}
...
function patchOutgoingRedirects(res) {
  ...
  res.setHeader = function patchedSetHeader(name, value) {
    return origSetHeader(name, rewriteRedirectHeader(name, value));
  };

  if (typeof res.appendHeader === "function") {
    const origAppend = res.appendHeader.bind(res);
    res.appendHeader = function patchedAppendHeader(name, value) {
      return origAppend(name, rewriteRedirectHeader(name, value));
    };
  }
  ...
  res.writeHead = function patchedWriteHead(statusCode, reason, headers) {
    ...
    if (hdrs && typeof hdrs === "object" && !Array.isArray(hdrs)) {
      for (const key of Object.keys(hdrs)) {
        hdrs[key] = rewriteRedirectHeader(key, hdrs[key]);
      }
    }
...
module.exports = { toPublicLocation, toPublicActionRedirect, patchOutgoingRedirects };
