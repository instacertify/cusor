...
export default function SearchBox({
  ...
  large?: boolean;
  placeholder?: string;
  /** Prefill from /search?q=… so the box matches the page */
  initialQuery?: string;
}) {
  ...
  /** Stable copy of the last searched query for footer link text */
  const lastQueryRef = useRef(initialQuery.trim());

  useEffect(() => {
    if (initialQuery && !q) setQ(initialQuery);
    // eslint-disable-next-line react-hooks/exhaustive-deps -- only sync once from URL
  }, [initialQuery]);
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      ...
      try {
        ...
        /* aborted or network — keep UI usable */
        if (id !== reqId.current) return;
        setResults([]);
        setEmptyHelp({
          notFound: true,
          message: `Couldn’t search for “${query}” right now`,
          tryQueries: ["cable", "LED lamp", "BIS certification", "pressure cooker"],
          browse: [
  ...
  function submit() {
    const query = q.trim() || lastQueryRef.current;
    ...
    if (!query) {
      // Never navigate to a blank dead search — open suggestions instead
      setEmptyHelp({
        notFound: true,
        message: "Type a product, IS number, or certification",
        tryQueries: ["cable", "LED lamp", "BIS certification", "pressure cooker", "BEE star"],
        browse: [
          { label: "Browse BIS products", href: "/products" },
          { label: "Certifications", href: "/certifications" },
          { label: "Product testing", href: "/testing" },
          { label: "Find a lab", href: "/search?type=labs" },
          { label: "Ask an expert", href: "/contact" },
        ],
        related: [],
      });
  ...
  const displayQuery = q.trim() || lastQueryRef.current;
  ...
  return (
    <div ref={boxRef} className="relative w-full">
      ...
      {showDropdown && results.length > 0 && displayQuery && (
        <div className="absolute z-40 mt-2 w-full max-h-[min(70vh,28rem)] overflow-y-auto bg-white rounded-2xl border border-cream-300 shadow-card-hover">
          ...
          <Link
            href={`/search?q=${encodeURIComponent(displayQuery)}`}
            onClick={() => setOpen(false)}
        ...
        <div className="absolute z-40 mt-2 w-full max-h-[min(70vh,32rem)] overflow-y-auto bg-white rounded-2xl border border-cream-300 shadow-card-hover">
          <div className="px-4 py-3 border-b border-cream-200 bg-cream-50">
            ...
            <p className="text-xs text-ink-600 mt-1">
              Try a related keyword below, or browse popular sections.
            </p>
          </div>
